...
def main():
    ...
    def wait_viewport(width, height):
        for _ in range(60):
            ...
            if abs(viewport["width"] - width) < 0.01 and abs(viewport["height"] - height) < 0.01:
                assert_page(f"innerWidth === {width} && innerHeight === {height}")
                return
            time.sleep(0.05)
    ...
    try:
        ...
        cases = [("none", 1856, 960), ("iphone-se", 375, 647), ("iphone-16-pro", 402, 790),
                 ("iphone-16-pro-max", 440, 872), ("ipad-mini", 744, 1087),
                 ("ipad-pro-11", 834, 1148), ("ipad-pro-13", 1032, 1330), ("macbook", 1440, 844)]
        for frame, width, height in cases:
        ...
        # Canvas resizing changes framing, never the device's layout or CSS targets.
        ...
        check("Portrait and minimum Canvas retain the phone viewport and native input")

        mcp.call("showtime_settings", canvas={"width": 1920, "height": 1080}, video={"width": 1280, "height": 720, "fps": 24})
        for frame in ["iphone-16-pro", "ipad-pro-11", "macbook"]:
            ...
            assert (stream["codec_name"], stream["width"], stream["height"], stream["r_frame_rate"]) == ("h264", 1280, 720, "24/1")
