...
3. Write a repeatable JSON film script. Top-level shape:
   ...
   Current frame: \(canvas.frame.title). Webpage viewport: \(Int(canvas.pageWidth)) × \(Int(canvas.pageHeight)) CSS pixels. Frames define screen proportions; the webpage uses the fitted screen area on the Canvas, with capture detail set by Export resolution. After changing frame, Canvas size, or inset, inspect again before choosing targets. Phone/tablet frames show device status UI instead of desktop browser chrome.
