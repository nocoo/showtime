...
struct DirectorActivityState {
    ...
    var mode: PlaybackMode = .design
    var name = "Your director is ready"
    var scene = "Design preview"
    var jobID: String?
    ...
    var json: [String: Any] {
        var result: [String: Any] = ["phase": phase.rawValue, "source": source.rawValue, "name": name, "scene": scene,
                                     "mode": mode.rawValue, "completed": completed, "total": total, "cue": cueNumber,
...
final class DirectorActivity: ObservableObject {
    ...
    func begin(_ script: FilmScript, id: String, source: DirectorActivitySource, mode: PlaybackMode, range: Range<Int>) {
        var next = DirectorActivityState()
        next.phase = .working
        next.source = source
        next.mode = mode
        next.name = mode == .design ? "Design preview" : script.name
        next.jobID = id
        next.total = range.count
        next.scene = mode == .design ? "Design preview" : "Script playback"
        if mode == .design { next.recent = state.recent }
        next.current = DirectorCue(id: id + ":prepare", symbol: "sparkles", title: "Setting the scene", detail: "Preparing the next shot.")
        state = next
    }

    func beginSetup(_ action: Action, index: Int, jobID: String) {
        var next = state
        next.scene = "Setup"
        next.current = DirectorCue(action, id: jobID + ":setup:\(index)")
        next.cueNumber = 0
        state = next
    }

    func beginCue(_ action: Action, index: Int, script: FilmScript, range: Range<Int>, jobID: String) {
        var next = state
        next.current = DirectorCue(action, id: jobID + ":\(index)")
        next.cueNumber = index + 1
        next.scene = state.mode == .design ? "Design preview" : "Script playback"
        if let marker = script.steps.prefix(index + 1).last(where: { $0.action == .marker }) {
            next.scene = marker.text ?? marker.label ?? "Current scene"
        }
        let upcoming = script.steps.enumerated().prefix(range.upperBound).dropFirst(index + 1).first(where: { $0.element.action != .marker })
        next.next = upcoming.map { DirectorCue($0.element, id: jobID + ":\($0.offset)") }
        state = next
    }

    func completeCue(completed: Int) {
        var next = state
    ...
    func inspected() {
        ...
        var next = state
        next.phase = .completed; next.source = .agent; next.name = "Design preview"; next.mode = .design; next.scene = "Design preview"
        ...
        next.current = DirectorCue(id: UUID().uuidString, symbol: "viewfinder", title: "Page inspected",
                                   detail: "Visible controls are ready to use in design previews and scripts.")
    ...
    func recordingStarted() {
        var next = DirectorActivityState()
        next.phase = .working; next.name = "Live recording"; next.mode = .record; next.source = .local
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 14) {
            HStack(spacing: 8) {
                ...
                Text(state.source == .agent ? "AI Director" : state.mode == .record ? "Recording" : "Rehearsal").font(.system(size: 13, weight: .semibold))
                ...
                HStack(spacing: 6) {
                    ...
                    Text(model.isFinishing ? "Saving film" : model.isRecording ? "Recording" : state.phase == .failed ? "Needs attention" : state.mode == .design ? "Design preview" : live ? (state.mode == .record ? "Preparing recording" : "Rehearsing") : "Finished")
            ...
            HStack(alignment: .center, spacing: 20) {
                ...
                VStack(alignment: .leading, spacing: 6) {
                    ...
                    Label(state.next?.title ?? (state.output != nil ? "Your film is ready to share" : state.mode == .design ? "Inspect the preview, then refine your script" : "End of the selected script"),
